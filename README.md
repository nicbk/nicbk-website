...
the Google sign-in round trip, the Lit Tracker's live sync, and the upload flow.
It starts its own Postgres, zero-cache, and Garage containers plus a patched app
server (on port 3100, so it can run alongside a dev server) and needs Docker.
...
Both serve <http://localhost:3000>, and both bring up six services: the
...
own one-shot `garage-init` job, the app, which starts only once both jobs have
...
zero-cache, real Garage) covering the sign-in flow, the Lit Tracker's live sync,
and the upload flow, and Conventional-Commits PR-title lint. The workflows
