 ...
 * `authors.ts`/`fields.ts`/`identifiers.ts` know the elements TEI shares
 * between a paper and a reference to one, and `header.ts`/`bibliography.ts`
 * know where in a document each of them lives.
 *
...
export type { PaperIdentifiers } from './identifiers'
